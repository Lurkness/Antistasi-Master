...
if (count _eligibles == 1 and (AS_commander in _eligibles)) exitWith {
	[[Petr, "hint", format["%1 tried to resign but is the only eligible commander, so it remains so", name AS_commander]], "AS_fnc_localCommunication"] call BIS_fnc_MP;
};
...
if !(isNull _bestCandidate) then {
	...
	sleep 5;
	[[Petr, "hint", _text], "AS_fnc_localCommunication"] call BIS_fnc_MP;
};
